...
public class WedoTextlyVisitor<T> extends CommonTextlyVisitor<T> {
    ...
    public T visitWeDoSensorExpr(ExprlyParser.WeDoSensorExprContext ctx) {
        ...
        switch ( sensor ) {
            case "gyroSensor":
                ...
                ExternalSensorBean externalSensorBeanGyro = new ExternalSensorBean(ctx.NAME().getText(), "TILTED", ctx.slot.getText().toUpperCase(), mutation, listHide);
                return (T) new SensorExpr(new GyroSensor(mkInlineProperty(ctx, "robSensors_gyro_getSample"), externalSensorBeanGyro));

            case "infraredSensor":

                mutation.setMode("DISTANCE");
                validateName(ctx.NAME().getText(), NameType.PORT);
                ExternalSensorBean externalSensorBeanInfra = new ExternalSensorBean(ctx.NAME().getText(), "DISTANCE", "- EMPTY_SLOT -", mutation, listHide);
    ...
    @Override
    public T visitRobotWeDoStatement(ExprlyParser.RobotWeDoStatementContext ctx) {
