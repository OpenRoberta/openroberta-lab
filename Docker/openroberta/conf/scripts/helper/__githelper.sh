...
# ----------------------- functions for an efficient workflow ----------------------------------
function pullDevelopAndRebaseActualBranch {
    local B=$(getBranchName)
	if [ -z "$(git status --porcelain)" ];
	then
		:
	else
		echo "please commit your changes in branch \"$B\" first"
		return
	fi
	...
	git checkout "$B"
	echo '!---------- rebase starts ----------!'
	local RS=$(git rebase develop)
	echo "$RS"
}

function pullDevelopAndRebaseManyBranches {
	if [ -z "$(git status --porcelain)" ];
	then
		:
	else
		echo "please commit your changes in branch \"$(getBranchName)\" first"
		return
	fi
	local B=$(getBranchName)
	echo '!---------- get develop changes ----------!'
	git checkout develop
	git pull
	for B do
		printf '!---------- %-19s ----------!\n' "rebase $B" 
		...
		then
			...
			echo "rebase FAILS and will be ABORTED. Rebase MANUALLY. Message from git was:"
			echo $RS
			git rebase --abort
		fi
	done
...
function commitInThisBranch {
	local B=$(getBranchName)
	if [ "$B" = "develop" ];
	then
		echo "this script must NOT be executed in the develop branch"
		return
	fi
	git add --all
	git status
...
function commitToBeAmendedInThisBranch {
	local B=$(getBranchName)
	if [ "$B" = "develop" ];
	then
		echo "this script must NOT be executed in the develop branch"
	...
	git add --all
	git status
	git commit -m 'TO BE AMENDED LATER'
}