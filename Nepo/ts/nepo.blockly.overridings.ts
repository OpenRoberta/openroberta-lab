...
Blockly.Procedures.isNameUsed = function (name: string, workspace: Blockly.Workspace, opt_exclude: Blockly.Block) {
	let blocks: Array<Blockly.Block> = workspace.getAllBlocks(false);
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		if ((blocks[i] as any).getProcedureDef) {
			...
			var procName = (procedureBlock as any).getProcedureDef();
