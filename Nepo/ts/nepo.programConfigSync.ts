 ...
 */
export function getConfigDropDown(name: string): Blockly.FieldDropdown {
    let ports: Array<Array<string>> = getConfigPorts(name);
 ...
 */
export function getConfigPorts(name: string): Array<Array<string>> {
    var ports: Array<Array<string>> = [];
    if (bricklyWorkspace) {
        var blocks: Array<Blockly.Block> = bricklyWorkspace.getAllBlocks(false);
        for (var x = 0; x < blocks.length; x++) {
            var func = (blocks[x] as any).getConfigDecl;
            if (func) {
                var config: Object = func.call(blocks[x]);
                if (config["type"] === name) {
