 ...
 * 
 **/
public class DifferentialPilotTest implements  MoveProvider,
		ArcRotateMoveController, MoveController {
	/**
	 ...
	 */
	public DifferentialPilotTest(final double wheelDiameter,
	 ...
	 *            running backward.
	 */
	public DifferentialPilotTest(final double leftWheelDiameter,
...
//		_right.addListener(this);
	...
	/**
	 * enables acting as listener to the motors and calles movementStopped() in
	 * its listeners when a movement ends
	 */
	public void listen() {
		_listening = true;
	}
...
//		System.out.println("movementStart");
        if(isMoving()) stop();
		if (_updatePending)movementStop();// causes update of pose 
	 ...
	 * moveStopped on listener.  Odometry Pose Provider will update its pose
	 */
	private synchronized void movementStop() {
		
//		float dist = getMovementIncrement();
//		float angle = getAngleIncrement();
		for (MoveListener ml : _listeners)
			ml.moveStopped(new Move(_type, getMovementIncrement(),
					getAngleIncrement(), _robotTravelSpeed, _robotRotateSpeed,
					isMoving()), this);
		_updatePending = false;
//		System.out.println("movementStop increments:"+dist+" "+angle);
	}
...
//		System.out.println("getMovement " + getMovementIncrement() + " "
//				+ getAngleIncrement());
