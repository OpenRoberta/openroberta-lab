...
/**
 * This container holds a lot of maps and sets extracted from annotations of AST
 * classes (starting with @Nepo...). These classes refer both to programs and
 * configurations, though the majority refers to program. A robot adds new data
 * to this container, when it has classes in the package
 * "de.fhg.iais.roberta.syntax" or below.
 */
public class AstFactory {
    private static final Logger LOG = LoggerFactory.getLogger(AstFactory.class);

    /**
     * maps a class name to the {@link BlockDescriptor}, that is extracted from the
     * annotations
     */
    private static final Map<String, BlockDescriptor> blockTypesByClassName = new HashMap<>();
    /**
     * maps a blockly name to the {@link BlockDescriptor}, that is responsible fpr
     * processing this block
     */
    private static final Map<String, BlockDescriptor> blockTypeByBlocklyName = new HashMap<>();
    /**
     * maps a field name used by blockly to describe a sensor (mode) usable in a get
     * sensor sample block to the sensors block description
     */
     ...
     */
    private static final String[] allLegalModesArray = { "ALTITUDE", "AMBIENTLIGHT", "ANALOG", "ANGLE", "BACKWARD", "CALIBRATE", "CALIBRATION", "CO2EQUIVALENT", "COLOUR", "COMPASS", "DATE", "DEFAULT", "DEGREE",
            "DIGITAL", "DISTANCE", "DOWN", "EDISON_CODE", "FACE_DOWN", "FACE_UP", "FOREWARD", "FREEFALL", "G3", "G6", "G8", "HUMIDITY", "IAQ", "IDALL", "IDONE", "LATITUDE", "LEFT", "LIGHT", "LIGHT_VALUE",
            "LINE", "LONGITUDE", "MODULATED", "NAMEALL", "NAMEONE", "NONE", "OBSTACLE", "OFF", "PM10", "PM25", "PRESENCE", "PRESSED", "PRESSURE", "PULSEHIGH", "PULSELOW", "RATE", "RCCODE", "RED", "REFLEXION",
            "RESET", "RGB", "RIGHT", "ROTATION", "SEEK", "SERIAL", "SHAKE", "SOUND", "SPEED", "START", "STOP", "STRENGTH", "TEMPERATURE", "TILTED", "TIME", "UNMODULATED", "UP", "UVLIGHT", "VALUE",
            "VOCEQUIVALENT", "WAIT_FOR_PRESS", "WAIT_FOR_PRESS_AND_RELEASE", "X", "Y", "Z" };
    /**
    ...
    /**
     * a map build from the data found in the @NepoConfiguration annotation of
     * configuration AST classes. It maps the blockly name of the configuration
     * block to an internal name that is used to generate the AST object (always of
     * type {@link ConfigurationComponent}, which represent the confuguration block.
     */
    ...
    public static void loadBlocks() {
        ...
        alreadyLoaded = true;
        try {
            ClassPath.from(ClassLoader.getSystemClassLoader()).getAllClasses().stream().filter(clazz -> clazz.getName().startsWith("de.fhg.iais.roberta.syntax.")).map(clazzInfo -> clazzInfo.load())
    ...
    public static List<Class<?>> getAstClasses() {
        return blockTypesByClassName.values().stream().map(BlockDescriptor::getAstClass).collect(Collectors.toList());
    }
    ...
    private static BlockDescriptor addToBlockTypeByBlocklyName(String name, String category, String[] blocklyNames, Class<?> astClass, F2M[] f2ms) {
        Assert.notNull(name);
        ...
        for (String blocklyName : blocklyNames) {
            ...
            if (!legalDuplicate) {
                Assert.fail(String.format("AST classes %s and %s mapped both blockly name %s. Initialization aborted", astClass.getName(), firstMapper.getAstClass().getName(), blocklyName));
            }
        }
        return blockDescriptor;
    }
