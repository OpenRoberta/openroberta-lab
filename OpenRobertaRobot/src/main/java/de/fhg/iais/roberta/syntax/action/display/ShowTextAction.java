...
public class ShowTextAction<V> extends Action<V> {
    ...
    public ShowTextAction(BlockType kind, BlocklyBlockProperties properties, BlocklyComment comment, Expr<V> msg, Expr<V> column, Expr<V> row, String port) {
    ...
    public ShowTextAction(Expr<V> msg, Expr<V> column, Expr<V> row, String port) {
        super(BlockTypeContainer.getByName("SHOW_TEXT_ACTION"));
        Assert.isTrue((msg != null) && (column != null) && (row != null));
        this.msg = msg;
        this.x = column;
        this.y = row;
