...
function start() {
  ...
  Blockly.inject(document.getElementById('blocklyDiv'),
          {path: '../', toolbox: toolbox, realtime: false,
           realtimeOptions:
...
function test(path) {
	...
	var xmlDomSource = Blockly.Xml.Roberta.textToDom(xmlTextSource);	
	removeAttributeDom(xmlDomSource);
	xmlTextSource = DomToString(xmlDomSource);
	
	Blockly.mainWorkspace.clear();
	Blockly.Xml.Roberta.domToWorkspace(Blockly.mainWorkspace, xmlDomSource);
	
	var xmlDomResult = Blockly.Xml.Roberta.workspaceToDom(Blockly.mainWorkspace);
