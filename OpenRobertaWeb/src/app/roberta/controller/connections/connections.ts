...
class AutoConnection extends AbstractPromptConnection {
    ...
    protected run(result) {
        GUISTATE_C.setState(result);
        /**TODO Filedownload does not work at the moment because only the sourceCode will get send back for partial Flashing instead of the generated hex code
         * see MbedCompilerWorker.java file
         */
        if (result.rc == 'ok') {
            let showPopup = false;
