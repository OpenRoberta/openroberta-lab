...
public class PythonCodeGeneratorVisitor implements MbedAstVisitor<Void> {
	...
	public static String generate(Configuration brickConfiguration, ArrayList<ArrayList<Phrase<Void>>> phrasesSet,
		...
		PythonCodeGeneratorVisitor astVisitor = new PythonCodeGeneratorVisitor(brickConfiguration, usedHardwareVisitor,
				0);
	...
	private static void generateCodeFromPhrases(ArrayList<ArrayList<Phrase<Void>>> phrasesSet, boolean withWrapping,
			PythonCodeGeneratorVisitor astVisitor) {
	...
	private static boolean handleMainBlocks(PythonCodeGeneratorVisitor astVisitor, boolean mainBlock,
			Phrase<Void> phrase) {
	...
	private boolean parenthesesCheck(Binary<Void> binary) {
		return binary.getOp() == Binary.Op.MINUS && binary.getRight().getKind().hasName("BINARY")
