...
/**
 * This class is implementing {@link IVisitor}. All methods are implemented and
 * they append a human-readable C++ code representation of a phrase to a
 * StringBuilder. <b>This representation is correct C++ code for Calliope
 * systems.</b> <br>
 */
public final class CalliopeCppVisitor extends AbstractCppVisitor implements IMbedVisitor<Void> {
	 ...
	 */
	public CalliopeCppVisitor(UsedHardwareBean usedHardwareBean, CodeGeneratorSetupBean codeGeneratorSetupBean,
			ConfigurationAst robotConfiguration, ArrayList<ArrayList<Phrase<Void>>> programPhrases) {
	...
	public Void visitDisplayImageAction(DisplayImageAction<Void> displayImageAction) {
		...
		if (displayImageAction.getDisplayImageMode().name().equals("ANIMATION")) {
			try {
				...
				this.sb.append("std::array<MicroBitImage, " + valuesSize
						+ "> _animation = _convertToArray<MicroBitImage, " + valuesSize + ">(");
	...
	public Void visitColorConst(ColorConst<Void> colorConst) {
		this.sb.append("MicroBitColor(" + colorConst.getRedChannelInt() + ", " + colorConst.getGreenChannelInt() + ", "
				+ colorConst.getBlueChannelInt() + ", " + 255 + ")");
		return null;
	}
	...
	public Void visitLightAction(LightAction<Void> lightAction) {
		...
		this.sb.append("_cbSetLed(_buf, &_i2c, _cbLedState, ").append(lightAction.getPort()).append(", ").append(mode)
				.append(");");
		return null;
	}
	...
	/*
	 * TODO: I don't know why I am doing this, but it seems that without this a
	 * semicolon is lost, somehow... Artem Vinokurov 25.10.2018
	 */
	@Override
	...
	/*
	 * TODO: There is something wrong with semicolon generation for calliope.
	...
	private void writeToSerial(Expr<Void> valueToWrite) {
		if (valueToWrite instanceof RgbColor<?> || valueToWrite instanceof ColorConst<?>
	...
	public Void visitAssertStmt(AssertStmt<Void> assertStmt) {
		if (((Binary<Void>) assertStmt.getAssert()).getLeft().getVarType().equals(BlocklyType.COLOR)) {
			...
			this.sb.append("\", \"").append(((Binary<Void>) assertStmt.getAssert()).getOp().toString())
					.append("\", \"");
