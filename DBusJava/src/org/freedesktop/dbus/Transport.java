/*
   ...
   Modified by Lawrie Griffiths to use JNA version of Unix sockets
...
import lejos.internal.io.NativeDomainSocket;
...
public class Transport
{
   public static class SASL
   {
      ...
      public boolean auth(int mode, int types, String guid, OutputStream out, InputStream in, NativeDomainSocket us) throws IOException
      {
   ...
   public void connect(BusAddress address, int timeout) throws IOException
   {
      ...
      if ("unix".equals(address.getType())) {
         ...
         if (null != address.getParameter("listen")) {
            ...
            NativeDomainSocket uss = new NativeDomainSocket();
            if (null != address.getParameter("abstract"))
               uss.bind(new NativeDomainSocket.SockAddr(address.getParameter("abstract"), true));
            else if (null != address.getParameter("path"))
               uss.bind(new NativeDomainSocket.SockAddr(address.getParameter("path"), false));
            us = uss.accept();
         } else {
            mode = SASL.MODE_CLIENT;
            us = new NativeDomainSocket();
            if (null != address.getParameter("abstract"))
               us.connect(new NativeDomainSocket.SockAddr(address.getParameter("abstract"), true));
            else if (null != address.getParameter("path"))
               us.connect(new NativeDomainSocket.SockAddr(address.getParameter("path"), false));
         }
         us.setPassCred(true);
