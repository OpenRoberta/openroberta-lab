...
/**
 * @fileoverview Object representing a mutator dialog. A mutator allows the user
 *               to change the shape of a block using a nested blocks editor.
 ...
 * 
 * @param {!Array.
 *            <string>} quarkNames List of names of sub-blocks for flyout.
...
/**
 * Clicking on the icon toggles if the mutator bubble is visible. Disable if
 * block is uneditable.
 * 
 * @param {!Event}
 *            e Mouse click event.
...
Blockly.Mutator.prototype.createEditor_ = function() {
  /*
   * Create the editor. Here's the markup that will be generated: <svg> <rect
   * class="blocklyMutatorBackground" /> [Flyout] [Workspace] </svg>
   */
...
/**
 * Callback function triggered when the bubble has resized. Resize the workspace
 * accordingly.
 * 
 * @private
 */
Blockly.Mutator.prototype.resizeBubble_ = function() {
  ...
  if (Math.abs(this.workspaceWidth_ - width) > doubleBorderWidth
    ...
    // Resize the bubble.
    this.bubble_.setBubbleSize(width + doubleBorderWidth, height
        + doubleBorderWidth);
...
Blockly.Mutator.prototype.setVisible = function(visible) {
  ...
  if (visible) {
    ...
    this.bubble_ = new Blockly.Bubble(this.block_.workspace, this
        .createEditor_(), this.block_.svg_.svgPath_, this.iconX_, this.iconY_,
        null, null);
    var thisObj = this;
    ...
    if (this.block_.saveConnections) {
      ...
      this.sourceListener_ = Blockly.bindEvent_(this.block_.workspace
          .getCanvas(), 'blocklyWorkspaceChange', this.block_, function() {
...
/**
 * Update the source block when the mutator's blocks are changed. Delete or bump
 * any block that's out of bounds. Fired whenever a change is made to the
 * mutator's workspace.
 * 
 * @private
 */
  ...
  if (Blockly.Block.dragMode_ == 0) {
    ...
    for (var b = 0, block; block = blocks[b]; b++) {
      ...
      if (block.isDeletable()
          && (Blockly.RTL ? blockXY.x > -this.flyout_.width_ + MARGIN
 ...
 * mutator flyout. The following properties are computed: .viewHeight: Height of
 * the visible rectangle, .absoluteTop: Top-edge of view. .absoluteLeft:
 * Left-edge of view.
 * 
