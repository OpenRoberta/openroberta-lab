...
Blockly.Xml.blockToDom_ = function(block, statement_list) {
    ...
    if (block.mutationToDom) {
        ...
        var mutation = block.mutationToDom();
        if (mutation) {
            if (mutation !== undefined && mutation != null && (block.type == 'procedures_defnoreturn' || block.type == 'procedures_defreturn')) {
                element.appendChild(repetitions);
                repe = true;
            }
        }
...
Blockly.Xml.childToBlock = function(workspace, block, xmlChild, opt_reuseBlock, blockChild) {
    ...
    case 'statement':
        ...
        blockChild = Blockly.Xml.domToBlock(workspace, RealGrandchildList, opt_reuseBlock);
