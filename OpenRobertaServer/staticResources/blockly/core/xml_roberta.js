...
goog.provide('Blockly.Xml.Roberta');
...
Blockly.Xml.Roberta.workspaceToDom = function (workspace) {
	...
	for (var i = 0, block; block = blocks[i]; i++) {
		...
		Blockly.Xml.Roberta.appendlistToDom(top, block);
 ...
 * @private
 */
Blockly.Xml.Roberta.blockToDom_ = function (block, statement_list) {
	...
	if (block.mutationToDom) {
		...
		if (mutation) {
			...
			if (mutation !== undefined && mutation != null && (block.type == 'controls_if'|| block.type == 'robControls_if' || block.type == 'robControls_ifElse')) {
	...
	for (var i = 0, input; input = block.inputList[i]; i++) {
		...
		if (input.type == Blockly.DUMMY_INPUT) {
			...
			if (childBlock) {
				Blockly.Xml.Roberta.appendlistToDom(container, childBlock);
				empty = false;
			}
		}
	...
	if (block.nextConnection) {
		...
		if (nextBlock) {
			Blockly.Xml.Roberta.blockToDom_(nextBlock, statement_list);
		}
	}
 ...
 * @param {!Element} parentDom a XML DOM element.
 * @param {!Object} block XML DOM element.
 */
Blockly.Xml.Roberta.appendlistToDom = function (parentDom, block) {
	var statement_list = [];
	Blockly.Xml.Roberta.blockToDom_(block, statement_list);
 ...
 */
Blockly.Xml.Roberta.domToWorkspace = function (workspace, xml) {
	var width = Blockly.svgSize().width;
	for (var x = 0, xmlTop; xmlTop = xml.childNodes[x]; x++) {
		if (xmlTop.nodeName.toLowerCase() == 'instance') {
			...
			var block = Blockly.Xml.Roberta.domToBlock(workspace, xmlChildList);
 ...
 * @param {!Element} xmlBlockList list of XML block elements.
 * @param {boolean=} opt_reuseBlock Optional arg indicating whether to
 *     reinitialize an existing block.
 * @return {!Blockly.Block} The root block created.
 * @private
 */
Blockly.Xml.Roberta.domToBlock = function (workspace, xmlBlockList, opt_reuseBlock) {
	for (var q = 0; q < xmlBlockList.length; q++) {
		...
		for (var x = 0, xmlChild; xmlChild = xmlBlock.childNodes[x]; x++) {
			...
			if (xmlChild.nodeName != 'repetitions')
				Blockly.Xml.Roberta.childToBlock(workspace, block, xmlChild, opt_reuseBlock, blockChild);
			else {
				for (var u = 0, xmlRepetChild; xmlRepetChild = xmlChild.childNodes[u]; u++) {
					...
					Blockly.Xml.Roberta.childToBlock(workspace, block, xmlRepetChild, opt_reuseBlock);
				}
			}
		}
		...
		if (nextBlockList.length != 0) {
			...
			blockChild = Blockly.Xml.Roberta.domToBlock(workspace, nextBlockList,
					opt_reuseBlock);
 ...
 * @param {!Blockly.Workspace} workspace The workspace.
 * @param {!Element} block XML block element.
 * @param {!Element} xmlBlock XML block element.
 * @param {boolean=} opt_reuseBlock Optional arg indicating whether to
 *     reinitialize an existing block.
 * @param {!Element} blockChild XML block element.
 */
Blockly.Xml.Roberta.childToBlock = function (workspace, block, xmlChild, opt_reuseBlock, blockChild) {
	...
	case 'statement':
		...
		blockChild = Blockly.Xml.Roberta.domToBlock(workspace, RealGrandchildList,
				opt_reuseBlock);
 ...
 */
Blockly.Xml.Roberta.textToDom = function(text) {
