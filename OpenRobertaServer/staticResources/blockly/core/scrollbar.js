 ...
 *            workspace Workspace to bind the scrollbars to.
 * @constructor
 */
...
/**
 * Recalculate both of the scrollbars' locations and lengths. Also reposition
...
Blockly.ScrollbarPair.prototype.resize = function() {
    ...
    var resizeH = false;
    var resizeV = false;
    if (!this.oldHostMetrics_ || this.oldHostMetrics_.viewWidth != hostMetrics.viewWidth || this.oldHostMetrics_.viewHeight != hostMetrics.viewHeight
            || this.oldHostMetrics_.absoluteTop != hostMetrics.absoluteTop || this.oldHostMetrics_.absoluteLeft != hostMetrics.absoluteLeft) {
        // The window has been resized or repositioned.
        resizeH = true;
        resizeV = true;
    } else {
        // Has the content been resized or moved?
        if (!this.oldHostMetrics_ || this.oldHostMetrics_.contentWidth != hostMetrics.contentWidth || this.oldHostMetrics_.viewLeft != hostMetrics.viewLeft
                || this.oldHostMetrics_.contentLeft != hostMetrics.contentLeft) {
            resizeH = true;
        }
        if (!this.oldHostMetrics_ || this.oldHostMetrics_.contentHeight != hostMetrics.contentHeight || this.oldHostMetrics_.viewTop != hostMetrics.viewTop
...
/**
 * Class for a pure SVG scrollbar. This technique offers a scrollbar that is
 * guaranteed to work, but may not look or behave like the system's scrollbars.
 * 
 * @param {!Blockly.Workspace}
 *            workspace Workspace to bind the scrollbar to.
 * @param {boolean}
 *            horizontal True if horizontal, false if vertical.
 * @param {boolean}
 *            opt_pair True if the scrollbar is part of a horiz/vert pair.
 * @constructor
 */
...
/**
 * Width of vertical scrollbar or height of horizontal scrollbar. Increase the
 * size of scrollbars on touch devices.
 */
Blockly.Scrollbar.scrollbarThickness = ('ontouchstart' in document.documentElement) ? 25 : 15;

/**
 * Dispose of this scrollbar. Unlink from all DOM elements to prevent memory
 * leaks.
 */
 ...
 * 
 * @param {Object=}
 *            opt_metrics A data structure of from the describing all the
 *            required dimensions. If not provided, it will be fetched from the
 *            host object.
 */
Blockly.Scrollbar.prototype.resize = function(opt_metrics) {
    ...
    /*
     * hostMetrics is an object with the following properties. .viewHeight:
     * Height of the visible rectangle, .viewWidth: Width of the visible
     * rectangle, .contentHeight: Height of the contents, .contentWidth: Width
     * of the content, .viewTop: Offset of top edge of visible rectangle from
     * parent, .viewLeft: Offset of left edge of visible rectangle from parent,
     * .contentTop: Offset of the top-most content from the y=0 coordinate,
     * .contentLeft: Offset of the left-most content from the x=0 coordinate,
     * .absoluteTop: Top-edge of view. .absoluteLeft: Left-edge of view.
     */
    if (this.horizontal_) {
...
/**
 * Create all the DOM elements required for a scrollbar. The resulting widget is
 * not sized.
 * 
 * @private
 */
Blockly.Scrollbar.prototype.createDom_ = function() {
    /*
     * Create the following DOM: <g> <rect class="blocklyScrollbarBackground" />
     * <rect class="blocklyScrollbarKnob" rx="7" ry="7" /> </g>
     */
...
/**
 * Scroll by one pageful. Called when scrollbar background is clicked.
 * 
 * @param {!Event}
 *            e Mouse down event.
 * @private
...
/**
 * Start a dragging operation. Called when scrollbar knob is clicked.
 * 
 * @param {!Event}
 *            e Mouse down event.
 * @private
...
/**
 * Constrain the knob's position within the minimum (0) and maximum (length of
 * scrollbar) values allowed for the scrollbar.
 * 
 * @param {number}
 *            value Value that is potentially out of bounds.
 ...
 * 
 * @param {number}
 *            value The distance from the top/left end of the bar.
 */
...
/**
 * Insert a node after a reference node. Contrast with node.insertBefore
 * function.
 * 
 * @param {!Element}
 *            newNode New element to insert.
 * @param {!Element}
 *            refNode Existing element to precede new node.
 * @private
 */
