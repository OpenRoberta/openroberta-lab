 ...
 *               including authorization and file loading. This functionality
 *               should serve mostly as a well-documented example, though is
...
/**
 * OAuth 2.0 scope for accessing the appdata folder, a hidden folder private to
 * this app.
 * 
 * @const
 */
 ...
 * 
 * @param {!Object}
 *            options Containing options.
 * @param {string}
 *            key Option key.
 * @param {*=}
 *            opt_defaultValue Default option value (optional).
 * @return {*} Option value.
 */
 ...
 * @constructor
 * @param {!Object}
 *            options For authorizer. Two keys are required as mandatory, these
 *            are:
 * 
 * 1. "clientId", the Client ID from the console 2. "authButtonElementId", the
 * is of the dom element to use for authorizing.
 */
rtclient.Authorizer = function(options) {
    ...
    this.authDiv = document.getElementById(rtclient.getOption(options, 'authDivElementId'));
};
 ...
 * 
 * @param {Function}
 *            onAuthComplete To call once authorization has completed.
 */
 ...
 * 
 * @param {Function}
 *            callback The callback to call after user ID has been fetched.
 */
 ...
 * @param {string}
 *            mimeType The MIME type of the new file.
 * @param {string}
 *            folderTitle Title of the folder to place the file in.
 * @param {Function}
 *            callback The callback to call after creation.
 */
 ...
 * 
 * @param {string}
 *            fileId The file to load metadata for.
 * @param {Function}
 *            callback The callback to be called on completion, with signature:
 * 
 ...
 * 
 * @param {string}
 *            stateParam The state query parameter as a JSON string.
 * @return {Object} The state query parameter as an object or null if parsing
 *         failed.
 */
 ...
 * @constructor
 * @param {!Object}
 *            options Options for loader. Four keys are required as mandatory,
 *            these are:
 * 
 * 1. "clientId", the Client ID from the console 2. "initializeModel", the
 * callback to call when the file is loaded. 3. "onFileLoaded", the callback to
 * call when the model is first created.
 * 
 * and two keys are optional:
 * 
 * 1. "defaultTitle", the title of newly created Realtime files. 2.
 * "defaultFolderTitle", the folder to place in which to place newly created
 * Realtime files.
 */
 ...
 * 
 * @param {Array.
 *            <string>} fileIds The IDs of the files to open.
 * @param {string}
...
rtclient.RealtimeLoader.prototype.redirectTo = function(fileIds, userId) {
    ...
    var newUrl = params.length == 0 ? window.location.pathname : (window.location.pathname + '#' + params.join('&'));
...
rtclient.RealtimeLoader.prototype.createNewFileAndRedirect = function() {
    ...
    // redirect to it.
    var _this = this;
    rtclient.createRealtimeFile(this.defaultTitle, this.newFileMimeType, this.defaultFolderTitle, function(file) {
        if (file.id) {
