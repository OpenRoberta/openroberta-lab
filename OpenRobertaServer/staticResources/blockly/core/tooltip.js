...
/**
 * @fileoverview Library to create tooltips for Blockly. First, call
 *               Blockly.Tooltip.init() after onload. Second, set the 'tooltip'
 *               property on any SVG element that needs a tooltip. If the
 *               tooltip is a string, then that message will be displayed. If
 *               the tooltip is an SVG element, then that object's tooltip will
 *               be used. Third, call Blockly.Tooltip.bindMouseEvents(e) passing
 *               the SVG element.
...
Blockly.Tooltip.createDom = function() {
    /*
     * <g class="blocklyHidden"> <rect class="blocklyTooltipShadow" x="2"
 ...
 * 
 * @param {!Element}
 *            element SVG element onto which tooltip is to be bound.
 */
...
/**
 * Hide the tooltip if the mouse is over a different object. Initialize the
 * tooltip to potentially appear for this object.
 * 
 * @param {!Event}
 *            e Mouse event.
 * @private
...
Blockly.Tooltip.wrapMutate_ = function(words, wordBreaks, limit) {
    ...
    for (var i = 0; i < wordBreaks.length - 1; i++) {
        ...
        var mutatedScore = Blockly.Tooltip.wrapScore_(words, mutatedWordBreaks, limit);
