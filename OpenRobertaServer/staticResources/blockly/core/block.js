...
Blockly.Block.prototype.initialize = function(workspace, prototypeName) {
	...
	if (goog.isFunction(this.onchange)) {
		Blockly.bindEvent_(workspace.getCanvas(), 'blocklyWorkspaceChange',
				this, this.onchange);
	}
};
...
Blockly.Block.terminateDrag_ = function() {
	...
	if (Blockly.Block.dragMode_ == 2) {
		...
		if (selected) {
			...
			selected.render();
			goog.Timer.callOnce(selected.bumpNeighbours_, Blockly.BUMP_DELAY,
					selected);
			...
			.doCommand(function() {
				...
				if (Blockly.selected && Blockly.highlightedConnection_) {
					...
					Blockly.localConnection_
							.connect(Blockly.highlightedConnection_);
					if (this_.svg_) {
						...
						inferiorConnection.sourceBlock_.svg_
								.connectionUiEffect();
					}
					if (this_.workspace.trashcan
							&& this_.workspace.trashcan.isOpen) {
						// Don't throw an object in the trash can if it just got connected.
						this_.workspace.trashcan.close();
					}
				} else if (this_.workspace.trashcan
						&& this_.workspace.trashcan.isOpen) {
	...
	if (this.isDeletable() && !block.isInFlyout) {
		...
		var deleteOption = {
			text : descendantCount == 1 ? Blockly.Msg.DELETE_BLOCK
					: Blockly.Msg.DELETE_X_BLOCKS.replace('%1',
...
Blockly.Block.prototype.setParent = function(newParent) {
	...
	if (newParent) {
		...
		if (newParent.svg_ && this.svg_) {
			newParent.svg_.getRootElement().appendChild(
...
Blockly.Block.prototype.setPreviousStatement = function(newBoolean, opt_check) {
	...
	if (newBoolean) {
		...
		this.previousConnection = new Blockly.Connection(this,
...
Blockly.Block.prototype.setNextStatement = function(newBoolean, opt_check) {
	...
	if (newBoolean) {
		...
		this.nextConnection = new Blockly.Connection(this,
...
Blockly.Block.prototype.setOutput = function(newBoolean, opt_check) {
	...
	if (newBoolean) {
		...
		this.outputConnection = new Blockly.Connection(this,
...
Blockly.Block.prototype.interpolateMsg = function(msg, var_args) {
	...
	goog.asserts.assert(dummyAlign === Blockly.ALIGN_LEFT
	...
	for (var i = 0; i < tokens.length; i += 2) {
		...
		if (symbol && symbol.charAt(0) == '%') {
			...
			goog.asserts.assertArray(tuple, 'Argument "%s" is not a tuple.',
					symbol);
			if (tuple[1] instanceof Blockly.Field) {
				fields.push([ tuple[0], tuple[1] ]);
			} else {
				input = this.appendValueInput(tuple[0]).setCheck(tuple[1])
						.setAlign(tuple[2]);
			}
...
Blockly.Block.prototype.moveNumberedInputBefore = function(inputIndex, refIndex) {
	...
	goog.asserts.assert(inputIndex < this.inputList.length, 'Input index '
			+ inputIndex + ' out of bounds.');
	goog.asserts.assert(refIndex <= this.inputList.length, 'Reference input '
			+ refIndex + ' out of bounds.');
	// Remove input.
