...
Blockly.Block = function() {
  ...
  goog.asserts
      .assert(arguments.length == 0, 'Please use Blockly.Block.obtain.');
};
...
Blockly.Block.prototype.onMouseDown_ = function(e) {
  ...
  if (Blockly.isRightButton(e)) {
    ...
    Blockly.Block.onMouseUpWrapper_ = Blockly.bindEvent_(document, 'mouseup',
  ...
  Blockly.doCommand(function() {
    ...
    if (Blockly.Block.dragMode_ == 2) {
      ...
      for (var i = 0; i < this_.draggedBubbles_.length; i++) {
        ...
        commentData.bubble.setIconLocation(commentData.x + dx, commentData.y
            + dy);
      }
 ...
 */
Blockly.Block.prototype.setColour = function(colourHue, colourSaturation,
    colourValue) {
  ...
  this.colourHex_ = goog.color.hsvToHex(this.colourHue_,
      this.colourSaturation_, this.colourValue_);
  this.updateColour();
};
...
Blockly.Block.prototype.interpolateMsg = function(msg, var_args) {
  ...
  for (var i = 0; i < tokens.length; i += 2) {
    ...
    if (symbol && symbol.charAt(0) == '%') {
      ...
      goog.asserts.assertArray(tuple, 'Message symbol "%s" is out of range.',
          symbol);
      ...
      if (tuple[1] instanceof Blockly.Field) {
        ...
        input = this.appendValueInput(tuple[0]).setCheck(tuple[1]).setAlign(
            tuple[2]);
      }
