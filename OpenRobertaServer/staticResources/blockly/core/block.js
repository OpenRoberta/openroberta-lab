 ...
 * @param {boolean}
 *            opt_dontRemoveFromWorkspace If true, don't remove this block from the workspace's list of top blocks.
 */
...
Blockly.Block.prototype.onMouseDown_ = function(e) {
    ...
    if (Blockly.isRightButton(e)) {
        ...
        // Allow unmovable blocks to be selected and context menued, but not dragged. Let this event bubble up to document, so the workspace may be dragged instead.
        return;
    } else {
...
Blockly.Block.prototype.onMouseUp_ = function(e) {
    ...
    Blockly.doCommand(function() {
        ...
        if (Blockly.selected && Blockly.highlightedConnection_) {
            ...
            // Dropping a block on the trash can will usually cause the workspace to resize to contain the newly positioned block. Force a second resize now that the block has been deleted.
            Blockly.fireUiEvent(window, 'resize');
        }
        ...
        if (Blockly.selected) {
            ...
            // Check if there is an error or warning to be removed
            if (Blockly.selected.warning != null && !Blockly.selected.warning.isVisible()) {
                Blockly.selected.warning.dispose();
                Blockly.selected.render();
            }
            if (Blockly.selected.error != null && !Blockly.selected.error.isVisible()) {
                Blockly.selected.error.dispose();
                Blockly.selected.render();
            }
        }
...
Blockly.Block.prototype.onMouseMove_ = function(e) {
    ...
    Blockly.doCommand(function() {
        if (e.type == 'mousemove' && e.clientX <= 1 && e.clientY == 0 && e.button == 0) {
             ...
             * This may result in a one-pixel blind spot in other browsers, but this shouldn't be noticeable.
             */
            e.stopPropagation();
            return;
        }
...
Blockly.Block.prototype.interpolateMsg = function(msg, var_args) {
    ...
    // Validate the msg at the start and the dummy alignment at the end, and remove the latter.
