 ...
 *            otherConnection Connection to connect to.
 */
 ...
 * 
 * @param {number}
 *            dx Change to x coordinate.
 * @param {number}
 *            dy Change to y coordinate.
 */
...
Blockly.Connection.prototype.highlight = function() {
    var steps;
    if (this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE) {
        steps = Blockly.BlockSvg.TAB_PATH_DOWN_SELECTED;
    } else {
        if (Blockly.RTL) {
            steps = 'm 20,0 h -5 l -6,4 -3,0 -6,-4 h -5'; //we do not use this, so no adoption
        } else {
            steps = Blockly.BlockSvg.NOTCH_PATH_LEFT_SELECTED //change from Beate
        }
    }
...
Blockly.Connection.prototype.closest = function(maxLimit, dx, dy) {
    ...
    function checkConnection_(yIndex) {
        ...
        if (connection.type == Blockly.INPUT_VALUE && connection.targetConnection && !connection.targetBlock().isMovable()) {
            return true;
        }
