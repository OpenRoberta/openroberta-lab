define(["require", "exports", "log", "jquery", "blockly"], function (require, exports, LOG, $, Blockly) {
    ...
    function displayMessage(messageId, output, replaceWith, opt_cancel, opt_robot) {
        var cancel = typeof opt_cancel !== 'string' ? opt_cancel : false;
        var robot = '';
        if (opt_robot) {
        ...
        if (messageId != undefined) {
            ...
            var value_1 = Blockly.Msg[messageId + robot] || Blockly.Msg[messageId];
            if (value_1 === undefined || value_1 === '') {
                value_1 = messageId;
            }
            if (typeof replaceWith === 'string') {
                if (value_1.indexOf('$') >= 0) {
                    value_1 = value_1.replace('$', replaceWith);
                }
                else {
                    value_1 = value_1.replace(/\{[^\}]+\}/g, replaceWith);
                }
            }
            else if (typeof replaceWith === 'object') {
                if (value_1.indexOf('$') >= 0) {
                    var keys = Object.keys(replaceWith);
                    value_1 = value_1.replace('$', replaceWith[keys[0]]);
                }
                else {
                    Object.keys(replaceWith).forEach(function (key) {
                        //@ts-ignore
                        if (replaceWith.hasOwnProperty(key)) {
                            value_1 = value_1.replace('{' + key + '}', replaceWith[key]);
                        }
                    });
                }
            }
            if (output === 'POPUP') {
                if (cancel) {
                    displayPopupMessage(lkey, value_1 + Blockly.Msg.POPUP_CONFIRM_CONTINUE, 'OK', Blockly.Msg.POPUP_CANCEL);
                }
                else {
                    displayPopupMessage(lkey, value_1, 'OK');
                }
            }
            else if (output === 'TOAST') {
                toastMessages.unshift(value_1);
