define(["require", "exports", "./interpreter.state", "./neuralnetwork.ui", "./interpreter.constants", "./interpreter.util", "simulation.roberta", "util.roberta"], function (require, exports, interpreter_state_1, neuralnetwork_ui_1, C, U, simulation_roberta_1, UTIL) {
    ...
    var Interpreter = /** @class */ (function () {
            ...
            if (stmt === undefined) {
                ...
                switch (opCode) {
                    ...
                    case C.COLOUR_COMPARE: {
                        var tolerance = this.state.pop();
                        var colour2 = this.state.pop();
                        var colour1 = this.state.pop();
                    ...
                    case C.OMNI_DRIVE: {
                        var thetaVel = this.state.pop();
                        var yVel = this.state.pop();
                        var xVel = this.state.pop();
                        this.robotBehaviour.omniDriveAction(xVel, yVel, thetaVel);
                        return [0, true];
                        break;
                    }
                    case C.OMNI_DRIVE_DIST: {
                        var distance = this.state.pop();
                        var yVel = this.state.pop();
                        var xVel = this.state.pop();
        ...
        Interpreter.prototype.evalExpr = function (expr) {
            ...
            switch (kind) {
                ...
                case C.BINARY: {
                    ...
                    var right = this.state.pop();
                    var left = this.state.pop();
                    var dataType = expr[C.TYPE]; // used by robots with integer arithmetic only
        ...
        Interpreter.prototype.evalBinary = function (subOp, left, right, dataType) {
