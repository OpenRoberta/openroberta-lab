...
define(["require", "exports", "jquery", "util.roberta", "blockly", "simulation.types"], function (require, exports, $, UTIL, Blockly) {
    ...
    var RobotBase = /** @class */ (function () {
        ...
        RobotBase.prototype.drawLabel = function (dt) {
            ...
            var $neuronValuesView = $('#neuronValuesView');
            $neuronValuesView.html('');
            var network = this.interpreter.neuralNetwork;
            if (network) {
                var neurons = network.getInputNames().concat(network.getHiddenNeuronNamesFlattened(), network.getOutputNames());
                if (Object.keys(neurons).length > 0) {
                    neurons.forEach(function (n) {
                        var neuronValue = network.getNeuronVal(n);
                        UTIL.addVariableValue($neuronValuesView, n, neuronValue);
                    });
                }
