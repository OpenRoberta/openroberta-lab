...
define(["require", "exports", "interpreter.constants", "simulation.math", "guiState.controller", "simulation.objects", "util.roberta", "jquery", "blockly"], function (require, exports, C, SIMATH, GUISTATE_C, simulation_objects_1, UTIL, $, Blockly) {
    ...
    var ChassisMobile = /** @class */ (function () {
        ...
        ChassisMobile.prototype.updateSensor = function (running, dt, myRobot, values, uCtx, udCtx, personalObstacleList, markerList, collisionList) {
            this.checkCollisions(running, this.id, myRobot.pose, dt, personalObstacleList, collisionList);
        };
    ...
    var ChassisDiffDrive = /** @class */ (function (_super) {
        ...
        function ChassisDiffDrive(id, configuration, maxRotation) {
            ...
            if (configuration['TRACKWIDTH'] === 0 && configuration['WHEELDIAMETER'] === 0) {
                // new configuration!
                for (var conf in configuration['ACTUATORS']) {
                    var myConf = configuration['ACTUATORS'][conf];
                    if (myConf['TYPE'] && myConf['TYPE'] === 'DIFFERENTIALDRIVE') {
                        _this.TRACKWIDTH = Number(myConf['BRICK_TRACK_WIDTH']) * 3;
                        _this.WHEELDIAMETER = Number(myConf['BRICK_WHEEL_DIAMETER']);
                        _this.right.port = myConf['MOTOR_R'];
                        _this.right.speed = 0;
                        _this.left.port = myConf['MOTOR_L'];
                        _this.left.speed = 0;
                    }
                }
                for (var conf in configuration['ACTUATORS']) {
                    var myConf = configuration['ACTUATORS'][conf];
                    if (myConf['TYPE'] && myConf['TYPE'] === 'MOTOR' && myConf['PORT'] !== _this.right.port && myConf['PORT'] !== _this.left.port) {
                        _this.manipulator = { port: myConf['PORT'], speed: 0, angle: 0 };
                    }
                }
            }
            else {
        ...
        ChassisDiffDrive.prototype.checkCollisions = function (running, myId, myPose, dt, personalObstacleList, collisionList) {
            var ground = personalObstacleList.slice(-1)[0]; // ground is always the last element in the personal obstacle list
            var groundBumped = false;
            ...
            var thetaDiff;
            var thisTolerance = Math.max(Math.abs(this.right.speed), Math.abs(this.left.speed));
            var _loop_1 = function (i) {
                ...
                var pointsInObstacle = [];
                pointsInObstacle = myCheckPoints.filter(function (checkPoint) {
                    if (SIMATH.checkInObstacle(myObstacle, checkPoint)) {
                        if (running && myObstacle instanceof simulation_objects_1.CircleSimulationObject && myObstacle.movable) {
                            return false;
                        }
                ...
                if (pointsInObstacle.length <= 0) {
                    ...
                    if (this_1['grabberLeft'] && this_1['grabberRight']) {
                        myCheckLines.push([this_1['grabberLeft'], this_1['grabberRight']]);
                        myCheckLines.push([this_1.frontLeft, this_1['grabberLeft']]);
                        myCheckLines.push([this_1.frontRight, this_1['grabberRight']]);
                    }
                    var p_1 = { x: 0, y: 0 };
                    if (!(myObstacle instanceof simulation_objects_1.CircleSimulationObject)) {
                        ...
                        myCheckLines.forEach(function (checkLine) {
                            for (var k = 0; k < obstacleLines_1.length; k++) {
                                ...
                                if (interPoint) {
                                    ...
                                    if (SIMATH.sqr(checkLine[2].rx - p_1.x) + SIMATH.sqr(checkLine[2].ry - p_1.y) < dt * (myObstacle.getTolerance() + thisTolerance)) {
                                        checkLine[0].bumped = true;
                                        checkLine[1].bumped = true;
                                    }
                                }
                        ...
                        myCheckLines.forEach(function (checkLine) {
                            var interPoint;
                            interPoint = SIMATH.getMiddleIntersectionPointCircle({ x1: checkLine[0].rx, x2: checkLine[1].rx, y1: checkLine[0].ry, y2: checkLine[1].ry }, myObstacle, 0);
                            if (interPoint) {
                                var bumped = true;
                                if (running && myObstacle.movable) {
                                    var circle = { x: myObstacle.x, y: myObstacle.y, r: myObstacle.r + dt * thisTolerance };
                                    var distP = SIMATH.getDistanceToCircle(interPoint, circle);
                                ...
                                if (bumped) {
                                    var dist0 = SIMATH.getDistance({ x: checkLine[0].rx, y: checkLine[0].ry }, interPoint);
                                    var dist1 = SIMATH.getDistance({ x: checkLine[1].rx, y: checkLine[1].ry }, interPoint);
                                    if (dist0 + 30 < dist1) {
                                        checkLine[0].bumped = true;
                                    }
    ...
    var RobotinoChassis = /** @class */ (function (_super) {
        ...
        RobotinoChassis.prototype.checkCollisions = function (running, myId, myPose, dt, personalObstacleList) {
            this.bumpedAngle = [];
    ...
    var EncoderChassisDiffDrive = /** @class */ (function (_super) {
        __extends(EncoderChassisDiffDrive, _super);
        function EncoderChassisDiffDrive() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        EncoderChassisDiffDrive.prototype.reset = function () {
            this.encoder.left = 0;
            this.encoder.right = 0;
            this.left.speed = 0;
            this.right.speed = 0;
        };
        EncoderChassisDiffDrive.prototype.updateAction = function (myRobot, dt, interpreterRunning) {
        ...
        EncoderChassisDiffDrive.prototype.updateSensor = function (running, dt, myRobot, values, uCtx, udCtx, personalObstacleList, markerList, collisionList) {
            _super.prototype.updateSensor.call(this, running, dt, myRobot, values, uCtx, udCtx, personalObstacleList, markerList, collisionList);
            if (running) {
                this.updateEncoders(running, values);
            }
        };
        EncoderChassisDiffDrive.prototype.updateEncoders = function (running, values) {
            if (running) {
                values.encoder = {};
        ...
        return EncoderChassisDiffDrive;
    }(ChassisDiffDrive));
    exports.EncoderChassisDiffDrive = EncoderChassisDiffDrive;
    ...
    var RCJChassis = /** @class */ (function (_super) {
        ...
        function RCJChassis(id, configuration, maxRotation, pose) {
            ...
            _this.grabberAngle = 0;
            _this.grabberWidth = _this.grabber.w;
            _this.grabberClosed = false;
            _this.topView = '<svg id="brick' +
                _this.id +
            ...
            _this.transformNewPose(pose, _this);
            _this.wheelLeft.w = _this.WHEELDIAMETER * 3;
            _this.wheelLeft.x = -_this.wheelLeft.w / 2;
            _this.wheelRight.w = _this.WHEELDIAMETER * 3;
            _this.wheelRight.x = -_this.wheelRight.w / 2;
            _this.wheelLeft.y = -_this.TRACKWIDTH / 2 - 3;
            _this.wheelRight.y = _this.TRACKWIDTH / 2 - 3;
            _this.wheelFrontRight.x = _this.wheelRight.x + _this.wheelRight.w;
            _this.wheelFrontRight.y = _this.wheelRight.y + _this.wheelRight.h;
            _this.wheelBackRight.x = _this.wheelRight.x;
            _this.wheelBackRight.y = _this.wheelRight.y + _this.wheelRight.h;
        ...
        RCJChassis.prototype.draw = function (rCtx, myRobot) {
        ...
        RCJChassis.prototype.getLabel = function () {
            return '';
        };
    ...
    exports.RCJChassis = RCJChassis;
    var Txt4Chassis = /** @class */ (function (_super) {
        __extends(Txt4Chassis, _super);
        function Txt4Chassis(id, configuration, maxRotation, pose) {
            var _this = _super.call(this, id, configuration, maxRotation) || this;
            _this.geom = {
                x: -28,
                y: -13,
            ...
            _this.wheelRight.w = _this.WHEELDIAMETER * 3;
            _this.wheelRight.x = -_this.wheelRight.w / 2;
            _this.wheelLeft.y = -_this.TRACKWIDTH / 2 - 4;
            _this.wheelRight.y = _this.TRACKWIDTH / 2 - 4;
            _this.wheelFrontRight.x = _this.wheelRight.x + _this.wheelRight.w;
            _this.wheelFrontRight.y = _this.wheelRight.y + _this.wheelRight.h;
            _this.wheelBackRight.x = _this.wheelRight.x;
            _this.wheelBackRight.y = _this.wheelRight.y + _this.wheelRight.h;
        ...
        Txt4Chassis.prototype.reset = function () {
            _super.prototype.reset.call(this);
            $('#display' + this.id).html('');
        };
        Txt4Chassis.prototype.updateAction = function (myRobot, dt, interpreterRunning) {
            _super.prototype.updateAction.call(this, myRobot, dt, interpreterRunning);
            var display = myRobot.interpreter.getRobotBehaviour().getActionState('display', true);
            if (display) {
                var yScale = 1.5;
                ...
                var $display = $('#display' + this.id);
                if (text) {
                    $display.html($display.html() + '<text x=0 y=' + ((y + 1) * 11) / yScale + ' transform="scale(' + xScale + ',' + yScale + ')">' + text + '</text>');
                }
                if (display.clear) {
                    $display.html('');
                }
        ...
        Txt4Chassis.prototype.draw = function (rCtx, myRobot) {
            _super.prototype.draw.call(this, rCtx, myRobot);
            rCtx.shadowBlur = 5;
            rCtx.shadowColor = 'black';
        ...
        Txt4Chassis.prototype.getLabel = function () {
            return ('<div><label>' +
                this.left.port +
                ' ' +
                Blockly.Msg['SENSOR_ENCODER'] +
    ...
    exports.Txt4Chassis = Txt4Chassis;
    ...
    var StatusLed = /** @class */ (function () {
        function StatusLed(location, geomColor, noneColor) {
            this.blink = 0;
    ...
    var MbedDisplay = /** @class */ (function (_super) {
        ...
        MbedDisplay.prototype.updateSensor = function (running, dt, myRobot, values, uCtx, udCtx, personalObstacleList, markerList, collisionList) {
    ...
    var Txt4RGBLed = /** @class */ (function (_super) {
        __extends(Txt4RGBLed, _super);
        function Txt4RGBLed(id, p, reset, port, radius) {
            var _this = _super.call(this, p, reset, port, radius) || this;
            _this.color = '#666666';
            _this.resetColor = '#666666';
            _this.id = id;
            return _this;
        }
        Txt4RGBLed.prototype.draw = function (rCtx, myRobot) {
            _super.prototype.draw.call(this, rCtx, myRobot);
            this.change();
        };
        Txt4RGBLed.prototype.change = function () {
            if (this.color !== this.resetColor) {
                $('#stopOn' + this.id).css({
    ...
    exports.Txt4RGBLed = Txt4RGBLed;
    var CalliopeV3RGBLeds = /** @class */ (function () {
        function CalliopeV3RGBLeds(positions, id) {
            this.rgbLeds = [];
            this.drawPriority = 88;
            for (var i = 0; i < positions.length; i++) {
                this.rgbLeds.push(new RGBLed(positions[i], true, i.toString(), 24));
            }
            this.robotId = id;
        }
        CalliopeV3RGBLeds.prototype.updateAction = function (myRobot, dt, interpreterRunning) {
            if (interpreterRunning) {
                var led_1 = myRobot.interpreter.getRobotBehaviour().getActionState('led', true);
                if (led_1) {
                    if (led_1['ALL']) {
                        if (led_1['ALL']['mode'] && led_1['ALL']['mode'] === 'off') {
                            for (var i = 0; i < this.rgbLeds.length; i++) {
                                this.rgbLeds[i].color = this.rgbLeds[i].resetColor;
                            }
                        }
                        else {
                            for (var i = 0; i < this.rgbLeds.length; i++) {
                                this.rgbLeds[i].color = led_1['ALL']['color'];
                            }
                        }
                    }
                    else {
                        var that_1 = this;
                        Object.keys(led_1).forEach(function (key, index) {
                            if (led_1[key]['mode'] && led_1[key]['mode'] === 'off') {
                                that_1.rgbLeds[key].color = that_1.rgbLeds[key].resetColor;
                            }
                            else if (that_1.rgbLeds[key] && led_1[key].color) {
                                that_1.rgbLeds[key].color = led_1[key].color;
                            }
                        });
                    }
                }
        ...
        CalliopeV3RGBLeds.prototype.draw = function (rCtx, myRobot) {
            for (var i = 0; i < this.rgbLeds.length; i++) {
                this.rgbLeds[i].draw(rCtx, myRobot);
            }
        };
        return CalliopeV3RGBLeds;
    }());
    exports.CalliopeV3RGBLeds = CalliopeV3RGBLeds;
