...
define(["require", "exports", "./neuralnetwork.helper", "./neuralnetwork.nn", "./neuralnetwork.uistate", "log", "./neuralnetwork.msg", "util.roberta", "neuralnetwork.linechart", "message", "jquery"], function (require, exports, H, neuralnetwork_nn_1, neuralnetwork_uistate_1, LOG, NN_MSG, UTIL, neuralnetwork_linechart_1, MSG, $) {
                    ...
                    case 1:
                        ...
                        D3.select('#nn-learn-run').on('click', function () { return __awaiter(_this, void 0, void 0, function () {
                            ...
                            return __generator(this, function (_a) {
                                ...
                                $runPauseIcon = $('#nn-learn-run span');
        ...
        fileInputEl.on('change', function (e) {
            ...
            if (file) {
                ...
                fileReader_1.onload = function () {
                    ...
                    if (forLearning) {
                        var numberOfNeededColumns = state.inputs.length + state.outputs.length;
                        if (!isInputDataValid_1(numberOfNeededColumns, inputData)) {
                        ...
                        var numberOfNeededColumns = state.inputs.length;
                        if (!isInputDataValid_1(numberOfNeededColumns, inputData)) {
                            MSG.displayMessage('NN_INVALID_TEST_TRAIN_DATA', 'POPUP', '', '', '');
                            inputData = file = null;
                            return;
                ...
                var isInputDataValid_1 = function (numberOfNeededColumns, inputData) {
                    if (inputData.length <= 0) {
                        return false;
                    }
                    return inputData.every(function (val, _, arr) { return val.length === numberOfNeededColumns; });
                };
            }
        });
    };
    ...
    var createUserInputTableBs = function (forLearning, tableDiv, tableEl, userInputs, inputTableNumRows) {
        if (userInputs === void 0) { userInputs = null; }
        tableDiv.show();
        if (forLearning) {
        }
        var neuronsToShow = forLearning ? __spreadArray(__spreadArray([], state.inputs, true), state.outputs, true) : __spreadArray([], state.inputs, true);
        var rowData = [];
        var columnData = [];
        function formatCellsAsInputs(value) {
            return "<input type='text' class='nn-table-input' value='".concat(value, "' data-neuron='").concat(this.field, "'>");
        }
        var updateTableCell = {
            'change input': function (e, value, row, index) {
                var attr = e.currentTarget.dataset.neuron;
        ...
        neuronsToShow.forEach(function (neuron) {
            var obj = {
                field: neuron,
        ...
        var appendTableRows = function (value) {
            var currentRow = {};
            neuronsToShow.forEach(function (neuron, idx) {
    ...
    function drawTheNetwork(tabType, tabSuffix, options) {
        ...
        for (var i = 0; i < numNodes; i++) {
            ...
            var cy = nodeStartY(i) + nodeDistanceFromTop;
        ...
        // Draw the output nodes separately.
        {
            ...
            for (var j = 0; j < numOutputs; j++) {
                ...
                var cy = nodeStartY(j) + nodeDistanceFromTop;
