                ...
                if (result.rc == "ok") {
                    if (GUISTATE_C.isProgramToDownload()) {
                        ...
                        var programLink = "<div id='programLink' style='text-align: center;'><br><a download='" + GUISTATE_C.getProgramName()
                                + ".hex' href='data:Application/octet-stream;content-disposition:attachment;charset=utf-8,"
                                + encodeURIComponent(result.compiledCode) + "'><strong>" + GUISTATE_C.getProgramName() + "</strong></a></div>";
                        var programHint = "<div id='programHint' class='popup-checkbox'><br><input type='checkbox' name='checkbox' id='confirmHide' value='value' /><label for='confirmHide'>Okay, I changed the download folder of my browser permanently. Don't show this popup again and download my programs directly.</label><div>";
                        $("#show-message").one("shown.bs.modal", function(e) {
                            $('#show-message div.modal-body').append(programLink);
                            $('#show-message div.modal-body').append(programHint);
                        });
                        $('#show-message').one('hidden.bs.modal', function(e) {
                            if ($('#confirmHide').is(':checked')) {
                                GUISTATE_C.setProgramToDownload();
                            }
                            $('#programLink').remove();
                            $('#programHint').remove();
                        });
                        MSG.displayPopupMessage('hallo', '<div class="download-checkbox">Please' + '<ul>' + '<li>right click on your program link and</li>'
                                + '<li>choose »Save link as ...«, then</li>' + '<li>click on »MICROBIT« in the left column and</li>'
                                + '<li>»Save«, that is all!</li>' + '</ul></div>', false);
                    }
                } else {
                    MSG.displayInformation(result, "", result.message, "");
                }
