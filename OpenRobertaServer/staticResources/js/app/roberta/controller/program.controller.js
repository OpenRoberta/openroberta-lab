define(
        [ 'exports', 'comm', 'message', 'log', 'util', 'simulation.simulation', 'guiState.controller', 'program.model', 'prettify', 'robot.controller',
                'tour.controller', 'blocks', 'jquery', 'jquery-validate', 'blocks-msg' ],
        function(exports, COMM, MSG, LOG, UTIL, SIM, GUISTATE_C, PROGRAM, Prettify, ROBOT_C, TOUR_C, Blockly, $) {
            ...
            function saveToServer() {
                ...
                PROGRAM.saveProgramToServer(GUISTATE_C.getProgramName(), GUISTATE_C.getProgramShared() ? true : false, GUISTATE_C.getProgramTimestamp(),
                        xmlText, function(result) {
                            if (result.rc === 'ok') {
                ...
                if (GUISTATE_C.isAutoconnected()) {
                    PROGRAM
                            .runOnBrickBack(
                                    ...
                                    xmlTextProgram,
                                    xmlTextConfiguration,
                                    function(result) {
                                        GUISTATE_C.setState(result);
                                        if (result.rc == "ok") {
                                            MSG.displayMessage("MESSAGE_EDIT_START", "TOAST", GUISTATE_C.getProgramName());
                                            //create link with content
                                            var programLink = '<div id="programLink" style="text-align: center;"><br><a download="'
                                                    + GUISTATE_C.getProgramName() + '.hex" href="data:text/plain;charset=utf-8,'
                                                    + encodeURIComponent(result.compiledCode) + '"><strong>' + GUISTATE_C.getProgramName()
                                                    + '</strong></a></div>';
                                            MSG
                                                    .displayPopupMessage(
                                                            'hallo',
                                                            'Please right click on your program link and choose »Save link as ...«, then click on »MICROBIT« in the left column and »Save«, that is all!',
                                                            false);
