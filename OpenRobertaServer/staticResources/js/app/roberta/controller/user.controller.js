define(["require", "exports", "message", "util.roberta", "user.model", "guiState.controller", "jquery", "blockly"], function (require, exports, MSG, UTIL, USER, GUISTATE_C, $, Blockly) {
    ...
    function createUserToServer() {
        ...
        if ($formRegister.valid()) {
            ...
            //@ts-ignore
            $('#registerAccountName').val(), $('#registerUserName').val(), $('#registerUserEmail').val(), $('#registerPass').val(), $('#registerUserAge').val(), GUISTATE_C.getLanguage(), function (result) {
                if (result.rc === 'ok') {
    ...
    function updateUserToServer() {
        ...
        $formRegister.validate();
        if ($formRegister.valid()) {
            USER.updateUserToServer(GUISTATE_C.getUserAccountName(), 
            //@ts-ignore
            $('#registerUserName').val(), $('#registerUserEmail').val(), $('#registerUserAge').val(), GUISTATE_C.getLanguage(), function (result) {
                if (result.rc === 'ok') {
                    //@ts-ignore
    ...
    function updateUserPasswordOnServer() {
        var restPasswordLink = $('#resetPassLink').val();
    ...
    function loginToUserGroup() {
        ...
        if ($formUserGroupLogin.valid()) {
            var values = $formUserGroupLogin.serializeArray(), valuesObj_1 = {};
            for (var i = 0; i < values.length; i++) {
                if (typeof values[i].name === 'undefined' || typeof values[i].value === 'undefined') {
                    continue;
                }
                valuesObj_1[values[i].name] = values[i].value;
            }
            USER.loginUserGroup(valuesObj_1.userGroupOwner, valuesObj_1.userGroupName, valuesObj_1.userGroupName + ':' + valuesObj_1.accountName, valuesObj_1.password, function (result) {
                if (result.rc === 'ok') {
                    $('#menuDeleteUser, #menuGroupPanel').parent().addClass('unavailable');
                    GUISTATE_C.setLogin(result);
                    MSG.displayInformation(result, 'MESSAGE_USER_LOGIN', result.message, GUISTATE_C.getUserName());
                    if (valuesObj_1.password === valuesObj_1.userGroupName + ':' + valuesObj_1.accountName) {
                        $('#passOld').val(valuesObj_1.password);
                        $('#grOldPassword').hide();
