define(["require", "exports", "message", "log", "guiState.controller", "program.controller", "robot.controller", "import.controller", "blockly", "util.roberta", "jquery", "bootstrap"], function (require, exports, MSG, LOG, GUISTATE_C, PROG_C, ROBOT_C, IMPORT_C, Blockly, U, $, bootstrap_1) {
    ...
    function shuffle(answers) {
        for (var j = void 0, x = void 0, i = answers.length; i; j = Math.floor(Math.random() * i), x = answers[--i], answers[i] = answers[j], answers[j] = x)
            ;
        return answers;
    }
    ...
    function openTutorialView() {
        ...
        if ($('#blocklyDiv').hasClass('rightActive')) {
            var waitForClose_1 = function () {
                if (!$('#blocklyDiv').hasClass('rightActive')) {
                    toggleTutorial();
                }
                else {
                    setTimeout(waitForClose_1, 50);
                }
            };
            waitForClose_1();
        }
