define([
  ...
  exports.loadFromTutorial = exports.chooseTutorial = exports.init = void 0;
  ...
  exports.chooseTutorial = chooseTutorial;
  ...
  function reloadTutorial(newLevel, opt_init) {
    ...
    tutorialId = tutorialList[newTutorialId]
      ? newTutorialId
      : tutorialId.slice(0, tutorialId.length - 2) + level + 1;
  ...
  function blocklyListener(event) {
    if (
      event.blockId !== "step_dummy" &&
      (event.newParentId || (event.name && event.name === "NUMBER")) &&
      blocklyWorkspace.remainingCapacity() == 0
    ) {
      if (
        checkSolutionCorrect(blocklyWorkspace.getAllBlocks()) === -4 &&
        !once
      ) {
        once = true;
        clearTimeout(myTimeoutID);
  ...
  function simTerminatedListener() {
    var blocks = blocklyWorkspace.getAllBlocks();
    var thisSolutionCorrect = checkSolutionCorrect(blocks);
    clearTimeout(myTimeoutID);
    switch (thisSolutionCorrect) {
      ...
      case Error.Direction: {
        ...
        if (thisSolutionCorrect === Error.Direction) {
          correct(
            "Es gibt einen kürzeren Weg. Überlege noch einmal und wähle den passenden Programmierbefehl!"
          );
        } else if (thisSolutionCorrect === Error.Collect) {
          correct(
            "Die Aufgabe des Roboters ist, die grünen Felder einzusammeln. Ergänze diesen Schritt!"
          );
      ...
      case Error.Steps: {
        setTimeout(function () {
          ...
          Blockly.hideChaff();
          MSG.displayMessage(
            "Wähle die richtige Anzahl an Feldern, die vorwärts gefahren werden sollen.",
            "POPUP",
            ""
          );
          $("#show-message").one("hidden.bs.modal", function () {
  ...
  function checkSolutionCorrect(blocks) {
    if (tutorial.step[step].solution) {
      for (var i = 0; i < tutorial.step[step].solution.length; i++) {
        var solution = tutorial.step[step].solution[i].split(",");
      ...
      if (tutorial.instructions) {
        ...
        if (tutorial.step[step].instruction) {
          if (tutorial.step[step].toolbox) {
            try {
              PROG_C.loadExternalToolbox(tutorial.step[step].toolbox);
              blocklyWorkspace.options.maxBlocks =
                tutorial.step[step].maxBlocks;
            } catch (e) {
              console.log(e);
            }
          }
          if (tutorial.step[step].header) {
            $("#tutorialContent").append(
              $("<h3>")
                .attr("class", "quiz header")
                .append(tutorial.step[step].header)
            );
          }
          ...
          if (tutorial.step[step].tip) {
            $("#tutorialContent")
              .append("<br><br>")
              .append($("<ul>").attr("class", "tip"));
            if (Array.isArray(tutorial.step[step].tip)) {
              for (var i = 0; i < tutorial.step[step].tip.length; i++) {
                $("#tutorialContent ul.tip").append(
                  "<li>" + tutorial.step[step].tip[i] + "</li>"
                );
              }
            } else {
              $("#tutorialContent ul.tip").append(
                "<li>" + tutorial.step[step].tip + "</li>"
              );
            }
          }
          if (tutorial.step[step].solution) {
    ...
    if (tutorial.step[step].quiz) {
      ...
      tutorial.step[step].quiz.forEach(function (quiz, iQuiz) {
        $("#tutorialContent .quiz.content").append(
          $("<div>").attr("class", "quiz question").append(quiz.question)
        );
    ...
    if (step < maxSteps) {
      ...
      if (step == maxSteps - 1 && quiz) {
        ...
        if (percent == 100) {
          ...
          $("#quizResult").append(
            " Du kannst die Quizfragen jederzeit wiederholen, wenn du möchtest!"
          );
        } else {
  ...
  function checkQuiz() {
    ...
    if (countCorrect !== totalCorrect) {
      ...
      if (countChecked == 0) {
        ...
        confirmText =
          countCorrect + " Anworten von " + totalCorrect + " sind richtig!";
      }
      $("#quizFooter").append(
        $("<span>", {
          text: confirmText,
