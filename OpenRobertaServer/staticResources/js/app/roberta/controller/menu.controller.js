define(["require", "exports", "message", "comm", "wrap", "user.controller", "notification.controller", "guiState.controller", "program.controller", "progRun.controller", "configuration.controller", "import.controller", "tour.controller", "sourceCodeEditor.controller", "progTutorial.controller", "util.roberta", "connection.controller", "blockly"], function (require, exports, MSG, COMM, WRAP, USER_C, NOTIFICATION_C, GUISTATE_C, PROGRAM_C, RUN_C, CONFIGURATION_C, IMPORT_C, TOUR_C, SOURCECODE_C, TUTORIAL_C, UTIL, CONNECTION_C, Blockly) {
    ...
    var Q_FORGOT_PASSWORD = 'forgotPassword';
    ...
    var Q_LOAD_PROGRAM = 'loadProgram';
    var Q_EXTENSIONS = 'extensions';
    var mainCallback;
    ...
    function handleQuery() {
        ...
        var oldStyle = (location.hash && location.toString().indexOf('#') <= domain.length + 1) || false;
        var newStyle = location.search !== '' || false;
        ...
        if (oldStyle) {
            ...
            var message = void 0;
            var germanDeprecated = 'Die eingegebenen URL-Parameter sind in dieser Form veraltet und werden bald nicht mehr unterstützt.\nBitte verwende ab sofort nur noch folgende Schreibweise:\n';
            var englishDeprecated = 'The URL parameters entered are outdated in this form and will soon no longer be supported.\n' +
                'Please use only the following spelling from now on:';
            var germanNotDeprecated = 'Die eingegebenen URL-Parameter sind in dieser Form veraltet und werden nicht mehr unterstützt.\nBitte verwende ab sofort nur noch folgende Schreibweise:\n';
            var englishNotDeprecated = 'The URL parameters entered are outdated in this form and are no longer supported.\n' + 'Please use only the following spelling from now on:';
        ...
        if (newStyle) {
            ...
            var tour_1 = getUrlParameter(Q_TOUR);
            if (tour_1) {
                GUISTATE_C.setStartWithoutPopup();
                mainCallback('ev3lejosv1', {}, function () {
                    PROGRAM_C.newProgram(true);
            ...
            var loadSystem_1 = getUrlParameter(Q_LOAD_SYSTEM);
            if (loadSystem_1 !== undefined) {
                GUISTATE_C.setStartWithoutPopup();
                var callback = void 0;
                var extend_1 = {};
                var parameter = [];
                ...
                if (tutorial) {
                    ...
                    extensionsArray.forEach(function (extension) {
                        ...
                        if (loadSystem_1 === 'calliope2017NoBlue' && extension === 'blue') {
                            loadSystem_1 = 'calliope2017';
                        }
                        else {
                            extend_1[extension] = true;
                        }
                    });
                }
                if (mainCallback && mainCallback instanceof Function) {
                    mainCallback(loadSystem_1, extend_1, callback, parameter);
                }
            }
        }
    }
    ...
    function initMenuEvents() {
        ...
        if (GUISTATE_C.isPublicServerVersion()) {
            var feedbackButton = '<div href="#" id="feedbackButton" class="rightMenuButton" rel="tooltip" data-bs-original-title="" title="">' +
        ...
        $('#head-navigation-program-edit').on('click', '.dropdown-menu li:not(.disabled) a', function (event) {
            var fn = function (event) {
                ...
                var targetId = target.id || currentTarget.id;
                switch (targetId) {
                    case 'menuRunProg':
                        RUN_C.runOnBrick();
                        break;
        ...
        $('.menuShowStart').onWrap('click', function () { return backToStartView(); });
