        ...
        if ($formLogin.valid()) {
            USER.login($("#loginAccountName").val(), $('#loginPassword').val(), function(result) {
                if (result.rc === "ok") {
                    ...
                    ROBERTA_NAVIGATION.setHeadNavigationMenuState('login');
                    ROBERTA_ROBOT.setState(result);
                }
                MSG.displayInformation(result, "MESSAGE_USER_LOGIN", result.message, userState.name);
            });
        }
    }
    ...
    function deleteUserOnServer() {
        ...
        if ($formSingleModal.valid()) {
            USER.deleteUserOnServer(userState.accountName, $('#singleModalInput').val(), function(result) {
                ...
                MSG.displayInformation(result, "MESSAGE_USER_DELETED", result.message, userState.name);
            });
        }
    }
