...
public class AstToTextlyVisitorTest {
    ...
    public void test2() throws Exception {
        ...
        String a = "{\n" //
            ...
            + "      UltraSonicSensor.setMode(S4, DISTANCE);\n"
            + "      LED.on(GREEN, ON);\n"
            + "   } else if ( 0 == ColorSensor.getValue(S3) ) {\n"
            + "      while ( true ) {\n"
    ...
    public void test3() throws Exception {
        ...
        String a = "{\n" //
            ...
            + "            if ( Bumper.isPressed(S1) ) {\n"
            + "                LED.on(GREEN, ON);\n"
            + "            } else if ( 0 == UltraSonicSensor.getValue(S4) ) {\n"
            + "                Display.drawPicture(FLOWERS, 15, 15);\n"
            + "            } else {\n"
            + "                UltraSonicSensor.setMode(S4, DISTANCE);\n"
            + "                while ( !Button.isPressedAndReleased(UP) ) {\n\n"
            + "                     Motor.on(B,30);"
            + "                }\n"
            + "            }\n"
            + "        }\n"
    ...
    public void test4() throws Exception {
        ...
        String a = "{\n" //
            ...
            + "            InfraredSensor.setMode(S4, SEEK);\n"
            + "            LED.off();\n"
            + "        } else {\n"
            + "            GyroSensor.reset(S2);\n"
