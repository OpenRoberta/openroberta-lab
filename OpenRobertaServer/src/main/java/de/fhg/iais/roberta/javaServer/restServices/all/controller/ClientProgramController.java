...
public class ClientProgramController {
	...
	public Response updateProject(@OraData HttpSessionState httpSessionState, JSONObject request) {
		...
		try {
			...
			if (dataPart.getString("cmd").equals("save")) {
				program = programProcessor.persistProgramText(programName, programText, configName, configText, userId,
						robot, userId, programTimestamp, !isShared);
			} else {
				program = programProcessor.persistProgramText(programName, programText, configName, configText, userId,
						robot, userId, null, true);
			}
	...
	public Response getProgram(@OraData HttpSessionState httpSessionState, JSONObject request) {
		...
		JSONObject response = new JSONObject();
		try {
			if (!httpSessionState.isUserLoggedIn() && !dataPart.getString("owner").equals("Roberta")
					&& !dataPart.getString("owner").equals("Gallery")) {
				...
				if (program != null) {
					...
					response.put("configName", program.getConfigName()); // may
																			// be
																			// null,
																			// if
																			// an
																			...
																			// is
																			// used
					response.put("configText", configText); // may be null, if
															// the default
															// configuration is
															// used
					response.put("lastChanged", program.getLastChanged().getTime());
		...
		try {
			if (!httpSessionState.isUserLoggedIn()) {
				...
				if (like) {
					...
					if (lp.succeeded()) {
						// nothing to do
						// argument: deleted tracks whether a like was set or
						// taken away
	...
	public Response deleteProjectShare(@OraData HttpSessionState httpSessionState, JSONObject request) {
		...
		try {
			...
			if (!httpSessionState.isUserLoggedIn()) {
				...
				UtilForREST.addResultInfo(response, accessRightProcessor);
				// if this program was shared from the gallery we need to delete
				// the copy of it as well
	...
	public Response createProjectShare(@OraData HttpSessionState httpSessionState, JSONObject request) {
		...
		try {
			...
			if (!httpSessionState.isUserLoggedIn()) {
				...
				// generating a unique name for the program owned by the
				// gallery.
				User user = userProcessor.getUser(userId);
				String userAccount = user.getAccount();
				if (!isPublicServer || user != null && user.isActivated()) {
					// get the program from the origin user to share with the
					// gallery
					...
					if (program != null) {
						if (program.getConfigName() == null) {
							...
							confText = configurationProcessor.getConfigurationText(program.getConfigName(), userId,
									robot);
						}
						// make a copy of the user program and store it as a
						// gallery owned program
						Program programCopy = programProcessor.persistProgramText(programName, program.getProgramText(),
								null, confText, galleryId, robot, userId, null, true);
						if (programProcessor.succeeded()) {
							if (programCopy != null) {
								response.put("lastChanged", programCopy.getLastChanged().getTime());
								// share the copy of the program with the origin
								// user
								accessRightProcessor.shareToUser(galleryId, robot, programName, userId, userAccount,
										"X_WRITE");
							} else {
