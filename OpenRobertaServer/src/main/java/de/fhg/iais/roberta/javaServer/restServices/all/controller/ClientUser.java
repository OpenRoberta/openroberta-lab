...
public class ClientUser {
	...
	@Inject
	public ClientUser(RobotCommunicator brickCommunicator, ServerProperties serverProperties,
	...
	public Response command(@OraData HttpSessionState httpSessionState, @OraData DbSession dbSession,
			JSONObject fullRequest) throws Exception {
		...
		try {
			...
			PendingEmailConfirmationsProcessor pendingConfirmationProcessor = new PendingEmailConfirmationsProcessor(
					dbSession, httpSessionState);
			if (cmd.equals("clear")) {
				...
				if (this.isPublicServer && !email.equals("") && up.succeeded()) {
					...
					PendingEmailConfirmations confirmation = pendingConfirmationProcessor
							.createEmailConfirmation(account);
				...
				if (user != null) {
					...
					String[] body = { user.getAccount(), lostPassword.getUrlPostfix() };
					try {
						this.mailManagement.send(user.getEmail(), "reset", body, lang, false);
						up.setStatus(ProcessorStatus.SUCCEEDED, Key.USER_PASSWORD_RECOVERY_SENT_MAIL_SUCCESS,
								responseParameters);
					} catch (MessagingException e) {
						up.setStatus(ProcessorStatus.FAILED, Key.USER_PASSWORD_RECOVERY_SENT_MAIL_FAIL,
								responseParameters);
					}
				}
				UtilForREST.addResultInfo(response, up);
				...
				PendingEmailConfirmations confirmation = pendingConfirmationProcessor
						.loadConfirmation(userActivationLink);
	...
	private void sendActivationMail(UserProcessor up, String urlPostfix, String account, String email, String lang,
			boolean isYoungerThen14) throws Exception {
		Map<String, String> responseParameters = new HashMap<>();
		String[] body = { account, urlPostfix };
		try {
