...
/**
 * <b>the main class of the application, the main activity is starting the
 * server.</b><br>
 * <br>
...
public class ServerStarter {
	 ...
	 *
	 * @param args a sequence of -d key=value and -D plugin:key=value for server
	 * resp. plugin property modifications
	 */
	public static void main(String[] args) throws Exception {
		initLoggingBeforeFirstUse(args);

		OptionParser parser = new OptionParser();
		OptionSpec<String> serverDefineOpt = parser.acceptsAll(asList("d", "server-property")).withRequiredArg()
				.ofType(String.class);
		OptionSpec<String> pluginDefineOpt = parser.acceptsAll(asList("D", "plugin-property")).withRequiredArg()
				.ofType(String.class);
	 ...
	 * @param serverDefines is a list of properties (from the command line ...)
	 * which overwrite the properties from the propertyPath. May be null.
	 */
	...
	/**
	 * startup. See {@link ServerStarter}. If the server could not be created,
	 * <b>the process will be terminated by System.exit(status) with status >
	 * 0</b>.
	 *
	 * @param pluginDefines
	 * @return the server
	...
	public Server start(List<String> pluginDefines) throws IOException {
		...
		if (httpPort > 0) {
			ServerConnector httpConnector = new ServerConnector(server); // NOSONAR
																			// :
																			// no
																			// need
																			// to
		...
		if (httpsPort > 0) {
			SslContextFactory sslContextFactory = new SslContextFactory(); // NOSONAR
																			// :
																			// no
																			// need
																			// to
		...
		RobertaGuiceServletConfig robertaGuiceServletConfig = new RobertaGuiceServletConfig(this.serverProperties,
				robotPluginMap, robotCommunicator, ipToCountry);
		...
		restHttpHandler.getSessionHandler().setMaxInactiveInterval(7200); // 2
																			// hours,
																			// should
																			// be
																			// ok
		...
		// see /rest endpoint, this will cover this:
		// restHttpHandler.getSessionHandler().addEventListener(mkSessionListener("
		// for / static resources, no REST endpoint"));
		...
		staticResourceServlet.setInitParameter("resourceBase",
				new File(dirNameStaticResources).toPath().toAbsolutePath().normalize().toUri().toASCIIString());
		staticResourceServlet.setInitParameter("cacheControl", "private, must-revalidate");

		HandlerList handlers = new HandlerList();
		handlers.setHandlers(new Handler[] { restHttpHandler, wsHandler, defaultHandler });
		...
		Runtime.getRuntime().addShutdownHook(new ShutdownHook(
				"embedded".equals(this.serverProperties.getStringProperty("database.mode")), this.injector));
		LOG.info(
				"Shutdown hook added. If the server is gracefully stopped in the future, a shutdown message is logged");
	...
	/**
	 * To get rid of Matomo, logging of the country code will be handled with
	 * the help of the ipToCountry class which maps the ip to the country code
	 * before something is logged.<br>
	 * In case the server is public make sure that there is database
	 * IpToCountry.csv available in the folder defined in the
	 * "server.iptocountry.dir" property, downloaded from <a href=
	 * "http://software77.net/geo-ip/?DL=1">http://software77.net</a></br>
	 * If "server.iptocountry.dir" property is provided a valid database is
	 * expected and will be used, regardless of whether the server is public or
	 * not.
	 *
		...
		if (pathIpToCountryDb != null) {
			try {
				...
				throw new DbcException(
						"Path to IpToCountry.cvs or IpToCountry.cvs seems to be invalid. Server does NOT start", e);
			}
	...
	/**
	 * First initialize logging. This is tricky, because we have to avoid any
	 * call to a method, that expects logging initialized already. In this case,
	 * a default initialization would occur. This is NOT wanted. The variables
	 * needed for logging are read from the args assuming a fixed layout (see
	 * "openRoberta.properties" for details)
	 *
	 * @param args the command line arguments. Needed to extract three optional
	 * logging parameter given by -d KEY=VAL
	 */
	public static void initLoggingBeforeFirstUse(String[] args) throws JoranException {
		...
		LOG = LoggerFactory.getLogger(ServerStarter.class); // here the
															// initialization
															// occurs
	...
	/**
	 * configure robot plugins, that may be used with this server. Uses the
	 * white list and the declarations from the openroberta.properties file.
	 *
	 * @param robotCommunicator
	 * @param pluginDefines modifications of plugin properties as a list of
	 * "<pluginName>:<key>=<value>"
	 * @return the mapping from robot names to the factory, that supplies all
	 * robot-specific data
	 */
	public static Map<String, IRobotFactory> configureRobotPlugins(RobotCommunicator robotCommunicator,
			ServerProperties serverProperties, List<String> pluginDefines) {
	...
	/**
	 * returns the guice injector configured in this class. This not dangerous,
	 * but you should ask yourself, why you need that ...</b>
	 *
	 * @return the injector
	 */
	 ...
	 * <b>Note:</b> the "hibernate.connection.url" property is added to the
	 * properties!
	 *
	 * @param properties for configuring OpenRoberta, merged from property file
	...
	private void setupPropertyForDatabaseConnection(Properties properties) {
		...
		if ("embedded".equals(databaseMode)) {
			dbUrl = "jdbc:hsqldb:file:" + databaseParentDir + "/db-" + serverVersionForDbDirectory + "/" + databaseName
					+ ";ifexists=true";
	...
	/**
	 * step through all configured robot factories and make sure, that the group
	 * name of each robot (or its name, if no group is configured) is in the
	 * database.<br>
	 * This is required, because the robot (group) name is used as foreign key
	 * in some tables
	 *
			...
			public void sessionDestroyed(HttpSessionEvent se) {
				...
				if (se == null) {
					...
					if (httpsession == null) {
						...
						HttpSessionState httpSessionState = (HttpSessionState) httpsession
								.getAttribute(OraDataProvider.OPEN_ROBERTA_STATE);
