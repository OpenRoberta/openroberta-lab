...
public final class SelectCamera<V> extends Action<V> {
    ...
    private SelectCamera(Camera camera, BlocklyBlockProperties properties, BlocklyComment comment) {
        super(BlockTypeContainer.getByName("SELECT_CAMERA"), properties, comment);
        Assert.notNull(camera, "Missing camera in selectCamera block!");
        this.camera = camera;
        setReadOnly();
    }

    /**
     * Creates instance of {@link SelectCamera}. This instance is read only and can not be modified.
     *
     * @param port {@link ActorPort} on which the motor is connected,
     * @param param {@link MotionParam} that set up the parameters for the movement of the robot (number of rotations or degrees and speed),
     * @param properties of the block (see {@link BlocklyBlockProperties}),
     * @param comment added from the user,
     * @return read only object of class {@link SelectCamera}
     */
    private static <V> SelectCamera<V> make(Camera camera, BlocklyBlockProperties properties, BlocklyComment comment) {
        return new SelectCamera<V>(camera, properties, comment);
    }
    ...
    protected V accept(AstVisitor<V> visitor) {
        return ((NaoAstVisitor<V>) visitor).visitSelectCamera(this);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, Jaxb2AstTransformer<V> helper) {
        ...
        return SelectCamera.make(Camera.get(camera), helper.extractBlockProperties(block), helper.extractComment(block));
    }
