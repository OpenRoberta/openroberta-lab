...
public final class TakePicture<V> extends Action<V> {
    ...
    private TakePicture(BlocklyBlockProperties properties, BlocklyComment comment) {
     ...
     */
    private static <V> TakePicture<V> make(BlocklyBlockProperties properties, BlocklyComment comment) {
        return new TakePicture<V>(properties, comment);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, Jaxb2AstTransformer<V> helper) {
        ...
        return TakePicture.make(helper.extractBlockProperties(block), helper.extractComment(block));
    }
