...
public class Jaxb2WeDoConfigurationTransformer {
    ...
    public BlockSet transformInverse(Configuration conf) {
        ...
        block.setType("robBrick_WeDo-Brick");
    ...
    private Configuration blockToBrickConfiguration(Block block) {
        switch ( block.getType() ) {
            case "robBrick_WeDo-Brick":
                ...
                return new WeDoConfiguration.Builder().setTrackWidth(trackWidth).setWheelDiameter(wheelDiameter).addActors(actors).addSensors(sensors).build();
            default:
