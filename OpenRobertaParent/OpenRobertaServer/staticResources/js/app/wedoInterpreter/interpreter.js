define([ 'exports', 'log', 'message', 'constants.interpreter', 'native.interpreter', 'state.interpreter', 'util.interpreter' ], function(exports, LOG, MSG,
     ...
     * 
     * @param generatedCode
     *            argument contains the operations and the function definitions .
     * @param cbOnTermination
     *            is called when the program has terminated
     */
    ...
    /**
     * force the termination of the program. The termination takes place, when
     * the NEXT operation should be executed. The delay is not significant. The
     * callbackOnTermination function is called
     */
    function terminate() {
        terminated = true;
    }
    ...
    /**
     * the central interpreter. It is a stack machine interpreting operations
     * given as JSON objects. The operations are all IMMUTABLE. It - uses the S
     * (state) component to store the state of the interpretation. - uses the N
     * (native) component for accessing hardware sensors and actors
     * 
     * if the program is not terminated, it will take one operation after the
     * other and execute it. The property C.OPCODE contains the operation code
     * and is used for switching to the various operations implementations. For
     * some operation codes the implementations is extracted to special
     * functions (repeat, expression) for better readability.
     * 
     * The state of the interpreter consists of - a stack of computed values -
     * the actual array of operations to be executed now, including a program
     * counter as index into the array - a stack of operations-arrays (including
     * their program counters), that are actually frozen until the actual array
     * has been interpreted. - a hash map of bindings. A binding map a name as
     * key to an array of values. This implements hiding of variables.
     * 
     * The stack of operations-arrays is used to store the history of complex
     * operation as - function call - if-then-else - repeat - wait - If such an
     * operation is executed, it pushes the actual array of operations
     * (including itself) onto the stack of operations-arrays, set the actual
     * array of operations to a new array of own operations (found at the
     * property C.STMT_LIST) and set the program counter to 0 - The program
     * counter of the pushed array of operations keeps pointing to the operation
     * that effected the push. Thus some operations as break have to increase
     * the program counter (to avoid an endless loop) - if the actual array of
     * operations is exhausted, the last array of operations pushed to the stack
     * of operations-arrays is re-activated
     * 
     * The statement C.FLOW_CONTROL is rather complex: - it is used explicitly
     * by 'continue' and 'break' and know about the repeat-statement /
     * repeat-continuation structure (@see eval_repeat()) - it is used
     * implicitly by if-then-else, if one branch is selected and is exhausted.
     * It forces the continuation after the if-then-else
     * 
     * Each operation code implementation may - create new bindings of values to
     * names (variable declaration) - change the values of the binding (assign) -
     * push and pop values to the stack (expressions) - push and pop to the
     * stack of operations-arrays
     */
    function evalOperation() {
        var _loop_1 = function() {
            ...
            case C.MOTOR_ON_ACTION: {
                ...
                if (duration >= 0) {
                    timeout(function() {
                        N.motorStopAction(name_2, port_1);
                        evalOperation();
                    }, duration);
                    return {
                        value : void 0
     ...
     * called from
     * 
     * @see evalOperation() to evaluate all kinds of expressions .
     * @param expr
     *            to be evaluated
     */
     ...
     * called from
     * 
     * @see evalOperation() to run a repeat statement
     * 
     * a repeat-statement ALWAYS contains a single repeat-continuation
     * statement. That in turn contains the body statements written by the
     * programmer. The repeat-statement does initialization of init, end, step
     * and the run variable for the FOR and TIMES variant (other variants don't
     * need that) The repeat-continuation is for updating the run variable in
     * the FOR and TIMES variant.
     * 
     * A continue statement pops the stack until a repeat-continuation is found
     * and re-executes it A break statement pops the stack until a
     * repeat-statement is found and skips that
     * 
     * Have a look at the functions for push and pop of operations in the STATE
     * component. The cleanup of the run variable is done there. This is not
     * optimal, as design decisions are distributed over two components. .
     * @param stmt
    ...
    function timeout(callback, durationInMilliSec) {
        if (terminated) {
            ...
            setTimeout(function() {
                timeout(callback, durationInMilliSec);
            }, 100);
        } else {
