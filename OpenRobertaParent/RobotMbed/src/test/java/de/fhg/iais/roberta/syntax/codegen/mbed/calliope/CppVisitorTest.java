...
public class CppVisitorTest {
    ...
    private static final String IMPORTS = //
        "#define_GNU_SOURCE\n\n"
            ...
            + "#include <array>\n"
            + "#include <stdlib.h>\n"
            + "#define SET_BRIGHTNESS_MULTIPLIER 255.0 / 9.0\n"
            + "#define GET_BRIGHTNESS_MULTIPLIER 9.0 / 255.0\n"
            + "#define ITERATION_SLEEP_TIMEOUT 1\n\n"
            + "MicroBituBit;"
    ...
    public void visitWaitStmt_TestTwoCases_ReturnsCorrectCppProgram() throws Exception {
        String expectedResult =
            "" //
                ...
                + "break;"
                + "}"
                + "uBit.sleep(ITERATION_SLEEP_TIMEOUT);"
                + "}"
                + "uBit.display.scroll(ManagedString(\"Hallo\"));"
                + "while(true){"
